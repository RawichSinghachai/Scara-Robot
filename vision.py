...
clicked_pos = None
show_result = True  # Flag to toggle between 'result' and 'mask'
...
def main():
    global clicked_pos, show_result
    # Open the webcam
    ...
    while True:
        ...
        # Show either 'result' or 'mask' based on the toggle flag
        if show_result:
            cv2.imshow('Color Detection', result)
        else:
            cv2.imshow('Color Detection', mask)

        # Handle the key press event for "i" and "o"
        ...
        if key == ord('o'):  # Press "o" to toggle between result and mask
