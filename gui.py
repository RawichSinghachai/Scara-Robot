...
from servo import (configure_servo, move_servo_smoothly, forward_servo360, backward_servo360, stop_servo360,servo_set_zero)
...
# ---------------------------------- Button Destination ---------------------------------------
        ...
        self.button_calculate_destination.clicked.connect(self.on_button_calculate_endpoint)  # Connect button click signal to a slot
...
# --------------------------- Switch Control -----------------------------------------------------------------
        ...
        self.button_set_zero.clicked.connect(self.on_button_set_zero)  # Connect button click signal to a slot
        self.hBox_bottom.addWidget(self.button_set_zero)
...
#       when click button start
    def on_button_start_control_mode(self):
        ...
        move_servo_smoothly(0,self.theta1_start,self.theta1_end,self.step,self.delay_servo)
        sleep(1)
        move_servo_smoothly(1,self.theta2_start,self.theta2_end,self.step,self.delay_servo)
        sleep(1)
        move_servo_smoothly(2,self.theta3_start,self.theta3_end,self.step,self.delay_servo)
        sleep(1)
        ...
        sleep(2)
        move_servo_smoothly(4,0,self.theta4_end,self.step,self.delay_servo)
        sleep(2)
...
#       when click calutation button
        ...
        self.theta1_end, self.theta2_end, self.theta3_end = inverse_kinematics(x,y+10) # y +10 because center offset 10 cm

        if self.theta1_end == None or self.theta2_end == None or self.theta3_end == None:
                self.theta1_end = str(self.theta1_end)
                self.theta2_end = str(self.theta2_end)
                self.theta3_end = str(self.theta3_end)
                QMessageBox.warning(self, "Warning", "Unable to move to destination")
        elif self.theta1_end > 270 :
                self.theta2_end = float(f"{(self.theta1_end - 270):.2f}")
                self.theta1_end = 270
                self.theta3_end = float(f"{self.theta3_end:.2f}")
                plot_robot_arm(x,y)
        elif self.theta1_end < 0 :
                self.theta2_end = float(f"{(90 + self.theta1_end):.2f}")
                self.theta1_end = 270
                self.theta3_end = float(f"{self.theta3_end:.2f}")
                plot_robot_arm(x,y)
        else:
                self.theta1_end = float(f"{self.theta1_end:.2f}")
                self.theta2_end = float(f"{self.theta2_end:.2f}")
                self.theta3_end = float(f"{self.theta3_end:.2f}")
                plot_robot_arm(x,y)
                

        self.label_angle_servo1.setText(f"Servo 1 : {self.theta1_end}")
        self.label_angle_servo2.setText(f"Servo 2 : {self.theta2_end}")
        self.label_angle_servo3.setText(f"Servo 3 : {self.theta3_end}")
...
#       when click calulate button endpoint
    def on_button_calculate_endpoint(self):
        ...
        self.theta1_start = self.theta1_des
        self.theta2_start = self.theta2_des
        self.theta3_start = self.theta3_des


        if self.theta1_des == None or self.theta2_des == None or self.theta3_des == None:
                self.theta1_des = str(self.theta1_des)
                self.theta2_des = str(self.theta2_des)
                self.theta3_des = str(self.theta3_des)
                QMessageBox.warning(self, "Warning", "Unable to move to destination")
    ...
    def on_button_confirm_endpoint(self):
        ...
        move_servo_smoothly(0,self.theta1_end,self.theta1_des,self.step,self.delay_servo)
        sleep(1)
        move_servo_smoothly(1,self.theta2_end,self.theta2_des,self.step,self.delay_servo)
        sleep(1)
        move_servo_smoothly(2,self.theta3_end,self.theta3_des,self.step,self.delay_servo)
        sleep(1)
        ...
        sleep(2)
        move_servo_smoothly(4,self.theta4_end,self.theta4_start,self.step,self.delay_servo)
        sleep(2)
    ...
    def on_button_reset(self):
        ...
        theta1_reset = self.theta1_des if self.theta1_des else self.theta1_end
        theta2_reset = self.theta2_des if self.theta2_des else self.theta2_end
        theta3_reset = self.theta3_des if self.theta3_des else self.theta3_end
        ...
        while True:
              ...
              sleep(0.1)
        sleep(1)
        move_servo_smoothly(0,theta1_reset,0,self.step,0.1)
        sleep(1)
        move_servo_smoothly(1,theta2_reset,0,self.step,0.1)
        sleep(1)
        move_servo_smoothly(2,theta3_reset,0,self.step,0.1)
        sleep(1)
        move_servo_smoothly(4,self.theta4_end,0,self.step,self.delay_servo)
        print("Reset")
